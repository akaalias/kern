...
- **2026-06-24** — **Snapshot harness landed** (`tests/snapshot.c` + 5 goldens in `tests/snapshots/`): plain/inline/inline-wrap/lists/headings rendered through the real wrap+md_draw_text path → deterministic serialized model diffed against committed goldens. Suite now **43 tests / 114 checks, green, 0 leaks.** App code still untouched.
