...
The pure core is effectively saturated (99.5% lines / 100% functions). The open frontier is `textview.c`, which the headless suite still doesn't compile, so its keydown handler, prefix chords (`C-x …`, ESC/meta), arrow glue, and the wikilink/buffer-switch modes are untested. Highest-value next step: **add a keydown test seam** — a function taking `(mod, sym, shift)` that runs the keydown decision logic against `g_ed`/`g_vs`, callable headlessly the way `unit_commands.c` drives `kern_dispatch_key` — then backfill prefix/arrow/wikilink feature tests on top of it. Secondary levers: a **fault-injection seam** (overridable `malloc`/`fopen`) to unlock the remaining error-path branches, and REFACTORING #3 (split `textview.c`) / #9 (unify prefix chords), both unblocked once the seam exists.
