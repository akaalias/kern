...
- **Style check** — `C-x s` toggles iA-Writer-style strike-through of cuttable text. A third per-line span layer (`style_check.c` → `Line.style_spans`, invalidated by `line_dirty` like `md_spans`/`pos_spans`); `md_draw_text` greys a struck word and draws a lightly-scribbled strike line (the `wave` wobble, one `r_draw_rect` per dash). Pure C — curated word/phrase lists (fillers + redundancies), whole-word/longest-match, no tagger or renderer dependency, fully headless-tested (`tests/unit_style.c`). Categories are `StyleCategory`; bit-per-category mask is `ViewState.style_mask` (0 = off). Detection is precision-first (context-free entries only); clichés/generalizations are a planned category.
