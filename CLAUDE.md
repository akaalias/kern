...
- **Style check** — `C-x s` toggles iA-Writer-style strike-through of cuttable text. A third per-line span layer (`style_check.c` → `Line.style_spans`, invalidated by `line_dirty` like `md_spans`/`pos_spans`). The decoration encodes the action the category implies (`StyleDecor`): fillers and the cuttable word of a redundancy get a greyed, lightly-scribbled **strike** through the x-height ("delete this"); clichés get a wavy **underline** at the baseline with text kept readable ("rewrite this"). A redundancy strikes only its cuttable word(s) (`cut_from`/`cut_len` per pattern) so the kept word stays unmarked ("a̶d̶d̶e̶d̶ bonus"). `md_draw_text` draws both as dashes with the `wave` wobble (one `r_draw_rect` per dash). Pure C — curated word/phrase lists (fillers, redundancies, clichés/generalizations) in `k_patterns[]`, whole-word/longest-match, no tagger or renderer dependency, fully headless-tested (`tests/unit_style.c`). Categories are `StyleCategory` (`STYLE_FILLER`/`STYLE_REDUNDANCY`/`STYLE_CLICHE`); bit-per-category mask is `ViewState.style_mask` (0 = off). Detection is precision-first (context-free entries only).
