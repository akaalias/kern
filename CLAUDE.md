...
- **Menu bar** — a **View** menu hosts "Syntax Highlighting" / "Style Check" on/off toggles (mirroring `C-x y` / `C-x s`). The items are declared via SwiftUI `.commands` (`App/KernApp.swift`) — the only way they stick, since SwiftUI rebuilds the standard menus — and call the `kern_toggle_*` bridges in `textview.c`. Live checkmarks can't come from SwiftUI (the parked main actor blocks updates), so an AppKit `NSMenuDelegate` on the View menu (`Platform/macos_style.m`) sets each item's ✓ in `menuNeedsUpdate:` from `kern_*_enabled()`, chaining to SwiftUI's delegate. Menu selection runs synchronously on the main thread during tracking, so the toggles fire despite the parked loop.
